...
def ccp_stock():
    original_recipes = sorted([{'display_name': f.split('_')[1].replace('.xlsx', '').replace('_', ' '), 'quantity_produced': f.split('_')[0][:-2], 'unit': f.split('_')[0][-2:]} for f in os.listdir('Recipes') if not f.startswith('~$')], key=lambda x: x['display_name'])

    # Load or initialize stock data
    stock_file = 'ccp_stock_data.xlsx'
    if os.path.exists(stock_file):
        stock_df = pd.read_excel(stock_file)
    else:
        stock_df = pd.DataFrame({'Product': [recipe['display_name'] for recipe in original_recipes], 'Current Stock (Kg)': [0] * len(original_recipes), 'Current Stock (Pieces)': [0] * len(original_recipes)})
        stock_df.to_excel(stock_file, index=False)

    if request.method == 'POST':
        data = request.form.to_dict(flat=False)
        for i, product in enumerate(data.get('product', [])):
            if data['update_stock'][i]:
                update_value = float(data['update_stock'][i])
                unit = data['unit'][i]
                if unit == 'kg':
    ...
    threshold_kg = request.args.get('threshold_kg', 10)
    threshold_pieces = request.args.get('threshold_pieces', 10)

    for record in stock_data:
        print(record.keys())  # Debug statement to print keys of the record
        if record.get('Current Stock (Kg)', 0) <= int(threshold_kg):
            record['current_stock_kg_style'] = 'color: red; font-weight: bold;'
        else:
            record['current_stock_kg_style'] = ''
        
        if record.get('Current Stock (Pieces)', 0) <= int(threshold_pieces):
            record['current_stock_pieces_style'] = 'color: red; font-weight: bold;'
        else:
            record['current_stock_pieces_style'] = ''

    return env.get_template('ccp_stock.html').render(original_recipes=original_recipes, stock_data=stock_data, threshold_kg=threshold_kg, threshold_pieces=threshold_pieces)
