...
def ccp_stock():
    ...
    if request.method == 'POST':
        ...
        if 'set_threshold' in request.form:
            threshold_kg = int(request.form.get('threshold_kg', 10))
            threshold_pieces = int(request.form.get('threshold_pieces', 10))
        else:
            threshold_kg = 10
            threshold_pieces = 10
    ...
    for record in stock_data:
        if record.get('Current Stock (Kg)', '') and float(record.get('Current Stock (Kg)', 0)) <= int(threshold_kg):
            record['current_stock_kg_style'] = 'color: red; font-weight: bold;'
        else:
            record['current_stock_kg_style'] = ''
        
        if record.get('Current Stock (Pieces)', '') and float(record.get('Current Stock (Pieces)', 0)) <= int(threshold_pieces):
