...
def op_stock():
    ...
    if request.method == 'POST':
        ...
        if 'update' in request.form:
            ...
            unit = request.form.get('unit')
            if unit == 'kg':
                stock_df.loc[stock_df['Product'] == product, 'Current Stock (Kg)'] += float(update_value)
            elif unit == 'pieces':
                stock_df.loc[stock_df['Product'] == product, 'Current Stock (Pieces)'] += float(update_value)
            stock_df.to_excel(stock_file, index=False)
            return redirect(url_for('op_stock', threshold_kg=threshold_kg, threshold_pieces=threshold_pieces))
