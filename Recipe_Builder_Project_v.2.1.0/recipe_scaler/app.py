...
os.makedirs('Final Product Recipes', exist_ok=True)
...
def save_dish_ingredients(dish_name, ingredients):
    dish_file_path = os.path.join('Final Product Recipes', f'{dish_name}.xlsx')
    df = pd.DataFrame(ingredients)
    df.to_excel(dish_file_path, index=False)
...
def create_dish():
    ...
    # Save the ingredient list in the Final Product Recipes folder
    ingredient_list = []
    for category, items in dropdown_data.items():
        for item in items:
            ingredient_list.append({'Category': category, 'Product': item['name'], 'Unit Cost': item['unit_cost']})

    final_product_path = os.path.join('Final Product Recipes', f'{dish_name}.xlsx')
    pd.DataFrame(ingredient_list).to_excel(final_product_path, index=False)
...
@app.route('/Created_Product_History', methods=['GET'])
def created_product_history():
    history_folder = 'Final Product Recipes'
    dishes = []
    if os.path.exists(history_folder):
        for filename in os.listdir(history_folder):
            if filename.endswith('.xlsx'):
                dishes.append(filename)
    return render_template('created_product_history.html', dishes=dishes)





def fetch_materials(path):
    materials = []
    if os.path.exists(path):
        df = pd.read_excel(path)
        if 'Product' in df.columns and 'Unit Cost' in df.columns:
            df['unit_cost'] = df['Unit Cost']
            materials = df[['Product', 'unit_cost']].to_dict('records')
    return materials
...
def save_ingredients():
    ...
    if dish_name and ingredients:
        file_path = os.path.join('Final Product Recipes', f'{dish_name}.xlsx')
        df = pd.DataFrame(ingredients, columns=['Ingredient', 'Quantity', 'Unit Cost'])
        df.to_excel(file_path, index=False)

    flash(f'Ingredients for "{dish_name}" saved successfully!', 'success')
    return redirect(url_for('price_setting_instructions'))
